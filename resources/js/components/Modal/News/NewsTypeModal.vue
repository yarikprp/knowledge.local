...
const props = defineProps<{
    modelValue: boolean;
    NewsTypeItem?: NewsTypeItemProps | null;
}>();

const emit = defineEmits(['update:modelValue', 'saved']);
const isLoading = ref(false);

const form = useForm<NewsTypeForm>({
    name: '',
});
...
watch(
    ...
    (value) => {
        dialog.value = value;
        if (value && props.NewsTypeItem) {
            form.name = props.NewsTypeItem.name;
        } else {
            form.reset();
        }
    },
...
const validationErrors = computed(() => {
    const errors: Partial<Record<keyof NewsTypeForm, string>> = {};
...
const submit = () => {
    ...
    if (props.NewsTypeItem?.id) {
        form.put(route('type.update', props.NewsTypeItem.id), {
            onSuccess: () => {
                emit('saved');
                dialog.value = false;
...
<template>
    <v-dialog v-model="dialog" max-width="600px" persistent>
        <v-card>
            <v-card-title>
                {{ NewsTypeItem ? 'Редактировать' : 'Добавить' }}
            </v-card-title>
