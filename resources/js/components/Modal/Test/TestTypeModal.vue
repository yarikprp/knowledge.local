...
const props = defineProps<{
    modelValue: boolean;
    TestTypeItem?: TestTypeItemProps | null;
}>();

const emit = defineEmits(['update:modelValue', 'saved']);
const isLoading = ref(false);

const form = useForm<TestTypeForm>({
    name: '',
});
...
watch(
    ...
    (value) => {
        dialog.value = value;
        if (value && props.TestTypeItem) {
            form.name = props.TestTypeItem.name;
        } else {
            form.reset();
        }
    },
...
const validationErrors = computed(() => {
    const errors: Partial<Record<keyof TestTypeForm, string>> = {};
...
const submit = () => {
    ...
    if (props.TestTypeItem?.id) {
        form.put(route('type.update', props.TestTypeItem.id), {
            onSuccess: () => {
                emit('saved');
                dialog.value = false;
...
<template>
    <v-dialog v-model="dialog" max-width="600px" persistent>
        <v-card>
            <v-card-title>
                {{ TestTypeItem ? 'Редактировать' : 'Добавить' }}
            </v-card-title>
